...
posterior_samples <- as.array(fit1)

#extract only p1 - p5
posterior_samples <- posterior_samples[, , c("p1", "p2", "p3", "p4", "p5")]

#rename
dimnames(posterior_samples)$parameters <- c("p_s", "lambda", "alpha", "rho", "varphi")


pars <- c("p_s", "lambda", "alpha", "rho", "varphi")

#plot chains
mcmc_dens_overlay(posterior_samples, pars = pars, facet_args = list(nrow = 2)) + xlim(0, NA)
