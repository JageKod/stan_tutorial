...
mod1 <- stan_model("model_part_pool_gq.stan")
...
pdf(sprintf("output_grid_%s_%d_part_pool_%s.pdf", experiment, number_of_patients, date), width = 21, height = 12)
...
for (patient_col in 1:no_reps){
  ...
  tsplot_cp <- ggplot(pred1_subset, aes(x = as.numeric(key), y = median)) +
    ...
    geom_ribbon(aes(ymin = lb, ymax = ub), fill = "green", alpha = 0.25) +
    geom_line(color = "green") + #median line
    geom_line(aes(y = lb), color = "green") +  # Lower bound line
    geom_line(aes(y = ub), color = "green")  # Upper bound line
  ...
  tsplot_cp <- tsplot_cp +
    labs(title = sprintf("PSA prediction for patient %d", patient_IDs[patient_col]),
         subtitle = sprintf("Partial pooling %s_%d", experiment, number_of_patients),
         y = "PSA (ug/L)",
         x = "Time (days)")
...
pdf(sprintf("output_patient_%d_%s_%d_part_pool_%s.pdf", patient_ID, experiment, number_of_patients, date))
...
# Add labels:
tsplot_cp <- tsplot_cp +
  labs(title=sprintf("PSA prediction for Patient %d", patient_ID), subtitle=sprintf("output_%s_part_pool_%s", experiment, date), y="PSA (ug/L)", x="time (days)", caption="")
