...
experiment <- 'all_but_last_cycle' # to load the correct patient data

number_of_patients <- 10  # to load the correct data. 10 or 70 available
...
# Get the number of sampling times and replicates
...
# Remove initial time and rename
...
nr_plot_points = nrow(plot_df)-1 # don't count row 0
...
mod1 <- stan_model("model_comp_pool.stan")
...
saveRDS(fit1, sprintf("fit_%s_%d_comp_pool_%s.rds",experiment, number_of_patients, date))
